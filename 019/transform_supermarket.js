...
console.log('🏪 开始转换 019-超市管理系统...\n');

// 转换 admin.html
let html = fs.readFileSync(path.join(__dirname, 'admin.html'), 'utf8');

// 基础替换
html = html.replace(/库存管理系统/g, '超市管理系统');
html = html.replace(/智慧仓储/g, '智慧超市');
...
const newMenu = `            <div class="nav-item active" onclick="showSection('dashboard')">
            ...
            <div class="nav-item" onclick="showSection('pos')">
...
// 统计卡片标题替换
html = html.replace(/总库存量/g, '今日销售额');
html = html.replace(/今日入库/g, '订单数量');
html = html.replace(/今日出库/g, '客单价');
html = html.replace(/库存预警/g, '会员数量');
...
mobileHtml = mobileHtml.replace(/库存管理/g, '超市管理');
mobileHtml = mobileHtml.replace(/智慧仓储/g, '智慧超市');
mobileHtml = mobileHtml.replace(/入库/g, '销售');
mobileHtml = mobileHtml.replace(/出库/g, '库存');
...
mobileJs = mobileJs.replace(/库存|仓储/g, '超市');
mobileJs = mobileJs.replace(/入库/g, '销售');
mobileJs = mobileJs.replace(/出库/g, '库存');
fs.writeFileSync(path.join(__dirname, 'mobile.js'), mobileJs, 'utf8');
console.log('✅ mobile.js 转换完成');

console.log('\n🎉 019-超市管理系统 转换完成！');
console.log('\n📦 核心模块:');
console.log('   ✅ 数据概览 - 销售趋势、分类占比');
console.log('   ✅ 商品管理 - 12个SKU、条码、价格、库存');
console.log('   ✅ 收银POS - 购物车、扫码加购、结算');
console.log('   ✅ 会员管理 - 8个会员、等级、积分、储值');
console.log('   ✅ 库存管理 - 预警、盘点');
console.log('   ✅ 供应商管理 - 6个供应商、采购管理');
console.log('   ✅ 销售报表 - 今日订单、销售统计');

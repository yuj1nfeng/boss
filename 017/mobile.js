...
const alertsMobile = [
    ...
    { id: 8, level: 'info', title: '今日灌溉完成', desc: '1-4号塘早餐灌溉已完成，共灌溉1370kg', time: '今天 09:30' }
];
    ...
    container.innerHTML = feedPlans.map((plan, index) => `
        <div class="feed-plan-item">
            ...
            <div class="feed-detail">
                <div class="feed-amount">灌溉 ${plan.amount}</div>
...
function toggleFeedPlan(index) {
    ...
    initFeedPlanList();
    showToast(feedPlans[index].enabled ? '已启用灌溉计划' : '已禁用灌溉计划');
}
...
console.log('%c 底部导航：首页、监测、灌溉、告警、我的', 'color: #94a3b8;');
